...
| 세션 4 (60분) | 진짜 도구로 교체 + 공개 MCP 연결 + 연쇄 마무리 | `server.py` 확장 |
...
```bash
uv add langgraph langchain langchain-mcp-adapters langchain-anthropic langchain-google-genai langchain-openai langchain-core python-dotenv
```
...
async def main() -> None:
    ...
    # 2) 클라우드 LLM에 Tool을 바인딩한다. (MODEL 문자열만 바꾸면 다른 모델로 전환된다)
    ...
    question = "과제 점수 90, 시험 점수 80이면 총점이 얼마야?"
    ...
    # 5) Resource 읽기 — Tool과 달리 LLM이 고르는 게 아니라 코드가 URI로 직접 읽는다.
    #    server.py의 @mcp.resource("course://{course_id}")는 {} 가 들어간 '템플릿'이라
    #    목록 조회에는 안 잡힌다. 아래처럼 실제 URI를 지정해야 읽을 수 있다.
    blobs = await client.get_resources("ai_master", uris="course://AI-MASTER")
    for blob in blobs:
        print("\n[Resource] course://AI-MASTER")
        print(blob.as_string())

    # 6) Prompt 가져오기 — 서버에 저장된 프롬프트 템플릿을 인자와 함께 받아온다.
    #    반환값이 이미 LangChain 메시지 리스트라서 그래프에 그대로 넣을 수 있다.
    prompt_messages = await client.get_prompt(
        "ai_master",
        "feedback_prompt",
        arguments={"student_name": "김민수", "topic": "MCP"},
    )
    print("\n[Prompt] feedback_prompt:", prompt_messages[0].content)

    feedback = await graph.ainvoke({"messages": prompt_messages})
    print("피드백 답변:", feedback["messages"][-1].content)
...
> **Tool · Resource · Prompt를 가져오는 방법이 각각 다릅니다.**
>
> | 종류 | 클라이언트 호출 | 누가 고르는가 |
> |---|---|---|
> | Tool | `await client.get_tools()` | **LLM**이 질문을 보고 선택 |
> | Resource | `await client.get_resources(서버명, uris=...)` | **코드**가 URI를 지정 |
> | Prompt | `await client.get_prompt(서버명, 프롬프트명, arguments={...})` | **코드**가 이름과 인자를 지정 |
>
> `get_resources`는 `uris`를 생략하면 서버가 목록으로 공개한 Resource만 가져옵니다.
> `course://{course_id}`처럼 `{}`가 들어간 템플릿 Resource는 목록에 나오지 않으므로
> `uris="course://AI-MASTER"`처럼 값을 채운 URI를 직접 넘겨야 합니다.
...
사용자 질의: 과제 점수 90, 시험 점수 80이면 총점이 얼마야?
최종 답변: 총점은 84.0점입니다. ...
...
[Prompt] feedback_prompt: 김민수 학생에게 MCP 학습 피드백을 작성하라. 잘한 점 1개, ...
피드백 답변: 김민수 학생은 ...
```
...
### C-5. 마무리 — 내 Tool과 공개 MCP Tool의 연쇄

세션 4의 두 방향이 **하나의 질문에서 만나는** 마무리 예제입니다. 6주차 1일차 A-4에서 만든
`get_youtube_search`를 `server.py`로 이사시키되(방향 ①), 한 가지를 바꿉니다 —
**검색 전용으로 다이어트**합니다. 자막은 공개 MCP의 `get_transcript`(방향 ②)에 맡깁니다.

왜 다이어트하는가? 6주차 버전과 비교하면 설계 원칙이 보입니다.

| | 6주차 방식 (뚱뚱한 도구) | 7주차 방식 (연쇄) |
|---|---|---|
| 구조 | 도구 하나가 검색 + 영상 3개 자막 전부 | 검색(내 서버) → LLM이 1개 선택 → 자막(공개 MCP) |
| 토큰 | 영상 3개 자막을 모두 소비 | **고른 영상 1개** 자막만 소비 |
| 교훈 | 일단 되게 만든다 | 도구를 잘게 나누면 에이전트가 골라 쓰며 아낀다 |

```bash
uv add youtube-search
```
...
```python
# server.py — Tool 추가 (6주차 A-4 함수에서 자막 로드 부분을 뺀 검색 전용 버전)
@mcp.tool()
def youtube_search(query: str, max_results: int = 3) -> str:
    """키워드로 유튜브 영상을 검색해 제목·길이·URL 목록을 반환한다. 자막은 가져오지 않는다."""
    videos = YoutubeSearch(query, max_results=max_results).to_dict()
    return "\n".join(
        f"- {v['title']} ({v['duration']}): https://youtube.com{v['url_suffix'].split('&')[0]}"
        for v in videos
    ) or "검색 결과가 없습니다."
```

`SYSTEM_PROMPT`에도 한 줄을 추가합니다:
`"유튜브 영상 검색은 youtube_search, 자막 추출은 get_transcript를 사용하라."`

서버를 재시작한 뒤(`Ctrl+C` 후 재실행), 연쇄 질문을 던집니다.

```python
    result = await graph.ainvoke(
        {"messages": [HumanMessage(content=(
            "유튜브에서 LangGraph 입문 영상을 검색해서 가장 적절한 영상 하나를 고르고, "
            "그 영상의 자막을 가져와 핵심 내용을 5줄로 요약해줘. 영상 URL도 함께 표시해줘."))]}
    )
```
...
```text
agent → youtube_search (내 server.py Tool)  → 영상 목록·URL
      → get_transcript (공개 MCP Tool)      → 고른 영상의 자막
      → agent                               → 5줄 요약 + URL 표시
```

이 한 번의 질문에서 에이전트가 **방향 ①(내가 만든 Tool)과 방향 ②(공개 MCP Tool)를 스스로
연쇄 호출**합니다. 6주차 2일차의 "환율 검색 → 계산기" 연쇄와 같은 원리이고, 오늘 배운 내용의
종합입니다 — 검색은 내 서버가, 자막은 공개 서버가, 판단과 요약은 LLM이 맡습니다.
...
- [ ] (C-5) 연쇄 질문에서 `youtube_search` → `get_transcript` 순서로 호출되고,
      최종 답변에 요약과 URL이 함께 나오나요?
...
async def build_graph():  # 세션 3과 동일한 그래프
    ...
    tools = await client.get_tools()
    model = init_chat_model(MODEL, temperature=0, max_tokens=1024).bind_tools(tools)
...
gr.ChatInterface(chat, title="내 MCP 에이전트").launch(
    server_name="0.0.0.0",       # 외부 기기 접속 허용
    server_port=8080,            # 8080 포트로 지정
    share=True,                  # 72시간 제한 외부 공유 링크 생성
    auth=("admin", "secret123"), # 로그인 인증 추가
    inbrowser=True               # 브라우저 자동 열기
)
```

> `launch()` 옵션은 필요한 것만 골라 쓰면 됩니다. 아무 인자 없이 `.launch()`만 쓰면
> 본인 PC에서만 `http://127.0.0.1:7860`으로 열립니다.
>
> | 옵션 | 효과 | 주의 |
> |---|---|---|
> | `server_name="0.0.0.0"` | 같은 공유기(Wi-Fi)의 다른 기기에서도 접속 가능 | 사내망·공용 Wi-Fi에서는 옆 사람도 들어올 수 있음 |
> | `server_port=8080` | 포트 고정 | 이미 쓰는 포트면 오류. MCP 서버(8000)와 겹치지 않게 |
> | `share=True` | Gradio가 임시 공개 주소(`*.gradio.live`)를 만들어 줌 | **72시간 후 만료**, 링크를 아는 사람은 누구나 접속 |
> | `auth=("admin", "secret123")` | 접속 시 아이디·비밀번호 요구 | 예시 비밀번호는 실제 공유 시 반드시 변경 |
> | `inbrowser=True` | 실행하면 브라우저가 자동으로 열림 | — |
>
> `share=True`로 만든 주소는 **내 PC로 들어오는 통로**입니다. 수업 시연이 끝나면 프로그램을
> 종료(`Ctrl+C`)해 링크를 닫으세요.
...
`inbrowser=True` 덕분에 브라우저가 자동으로 열립니다. 주소는 `http://127.0.0.1:8080`이고,
로그인 창이 뜨면 `admin` / `secret123`을 입력합니다. 터미널에는 `share=True`가 만든
`https://....gradio.live` 공개 주소도 함께 출력됩니다.

채팅창에 "과제 90 시험 80이면 총점은?", "생성형 AI 최신 사례 검색해줘"를 입력해 보세요.
...
- [ ] (C-5) `youtube_search` → `get_transcript` 연쇄 호출 실행 로그 (요약 + URL)
...
- **`youtube_search`·`get_transcript`가 네트워크 오류를 내요** → 학교·회사 네트워크가
  유튜브를 차단하면 실패합니다. 개인 네트워크에서 시도하세요. `youtube-search` 패키지가
  유튜브 개편으로 깨지면 검색 부분만 `yt-dlp`(`ytsearchN:검색어`)로 교체하면 됩니다.
