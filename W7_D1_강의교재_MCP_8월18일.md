...
그림으로 보면, **에이전트 루프(`agent ↔ tools`)는 6주차와 똑같고 Tool만 앱 밖의 표준 서버로 빠져나온다.**
...
즉 새로 배우는 것은 "**Tool을 어떻게 서버로 분리하고 표준으로 부르는가**"이고, 6주차에서 익힌
`bind_tools`·`ToolNode`·`agent ↔ tools` 루프는 그대로 재사용한다.
...
├── pyproject.toml
├── uv.lock
├── server.py         # (5장) Tool·Resource·Prompt를 제공하는 MCP 서버
├── agent_client.py   # (6.2) LLM 에이전트가 MCP Tool을 스스로 골라 쓰는 클라이언트 ★핵심
├── client.py         # (6.3·심화) 원시 JSON-RPC로 연결·호출만 확인하는 클라이언트
├── tools_server.py   # (6.4) 6주차의 검색·RAG·메모리 도구를 MCP로 감싼 서버
└── app.py            # (6.5) Gradio 최소 챗 UI (에이전트 그래프 재사용)
```

> 이 7주차 실습은 6주차의 `week6-agent`와 **별개의 새 프로젝트(`mcp-demo`)**로 만든다. 6주차 코드를
> 건드리지 않고, 그 안의 도구(함수)만 MCP 서버로 옮겨 온다(6.4).

## 1.4 실습 환경 준비 (먼저 확인)

실습에서 막히는 대부분은 개념이 아니라 **환경 설치**다. 시작 전에 아래를 확인한다.

| 도구 | 필수 여부 | 확인 / 준비 |
|---|---|---|
| `uv` (+ `uvx` 포함) | ✅ 필수 | `uv --version`, `uvx --version` |
| Python 3.11+ | ✅ 필수 | `python --version` |
| Anthropic API Key | ✅ 필수(클라우드 LLM) | `.env`에 `ANTHROPIC_API_KEY=sk-ant-...` |
| Node.js 18+ (`npx`) | ⛳ 선택 | 7장 확장(Weather·Finance·Filesystem·Memory·Playwright)에만 필요 |
| Ollama | ⛳ 선택 | 클라우드 대신 로컬 무료 LLM을 쓸 때만 |

> **최소 성공 경로 (Node.js 불필요)**: 5·6장의 `server.py`·`client.py`·`agent_client.py`는 모두
> 순수 Python이고, 7장도 `Fetch`·`Time`(uvx)만으로 완주할 수 있다. 즉 **Node.js 없이** 오늘 실습의
> 핵심을 처음부터 끝까지 경험할 수 있다. Node 기반 공개 서버는 관심 있는 학생만 추가로 설치한다.
...
# 제6장. 실습 B — 내 MCP 서버를 에이전트에 연결하기

## 6.1 이 장의 목표

> **원시 호출은 이해용, 실제 확장은 에이전트로.** 5장에서 만든 서버(`server.py`)가 공개한 Tool을
> 소비하는 방법은 두 가지다. **사람이 직접 `call_tool`을 부르는 원시 클라이언트**는 프로토콜이 어떻게
> 도는지 눈으로 보는 **이해용(심화·선택)**이고, **LLM이 Tool을 스스로 고르는 에이전트**는 이후 주차로
> 이어 갈 **실제 확장의 출발점(핵심)**이다. 그래서 이 장은 **에이전트를 먼저 만들고**(6.2), 원시
> 클라이언트는 심화로 둔다(6.3).

`server.py`와 클라이언트는 **서로 독립된 프로그램**으로, 터미널 2개에서 Streamable HTTP로 통신한다.
...
[터미널 1] server.py  →  MCP Server (http://localhost:8000/mcp)
                              ...
                              ▼
[터미널 2] agent_client.py (또는 client.py)
```

- 서버 터미널은 실습 내내 켜 둔다. 클라이언트가 종료돼도 서버는 계속 돈다.
- 실습이 끝나면 서버 터미널에서 `Ctrl+C`로 종료한다.

### (참고) 6주차 동기 코드 → 7주차 비동기 코드

이 장부터 `async`/`await`가 등장하지만, 6주차 동기 호출과 1:1로 대응되니 새 문법 3가지만 기억하면 된다.
...
| (없음) | `async with Client(url) as client:` — 연결을 자동으로 열고 닫음 |

규칙은 셋뿐이다. **① `async def` 안에서만 `await`를 쓴다. ② 최상위 실행은 `asyncio.run(main())`.
③ (원시 클라이언트의) 연결 수명은 `async with`가 관리한다.** MCP 통신은 네트워크 I/O라 비동기로 처리한다.

## 6.2 [핵심 실습] 에이전트가 내 MCP 툴을 사용한다 — `agent_client.py`

강의 목표인 **"자작·공개 MCP 툴을 LLM 에이전트가 스스로 골라 쓰게 만들기"**를 5장에서 만든
`server.py`로 그대로 실습한다.

### 왜 이 구조인가

- MCP 서버가 Tool을 표준으로 공개하므로, 에이전트는 `get_tools()` 한 번으로 Tool을 **자동 발견**한다.
  Tool마다 연결 코드를 새로 짜지 않는다 — 이것이 "툴 개발을 쉽게 한다"의 핵심이다.
- `agent ↔ tools` 루프는 **6주차 2일차에서 만든 것과 같다.** 새로 배우는 건 "서버에서 Tool을
  자동으로 받아온다"는 부분뿐이다. 흐름은 7장(여러 공개 MCP 연결)과도 동일하다:
  **MCP 서버 → get_tools → LLM 바인딩 → LangGraph 루프**.

```text
[터미널 1] server.py (5장에서 만든 MCP 서버, http://localhost:8000/mcp)
                     ▲
                     │ streamable_http + JSON-RPC
                     ▼
...
`mcp-demo` 프로젝트에 에이전트용 패키지를 추가한다. (LLM은 클라우드 `ChatAnthropic`을 사용)
...
먼저 5장의 서버를 켜 두고(터미널 1), 새 터미널에서 에이전트를 실행한다.
...
핵심은 `agent_client.py`가 `calculate_score()` 함수를 직접 import하지 않는다는 것이다. LLM이 자연어
질문을 보고 어떤 Tool을 쓸지 정하면, 실제 함수 실행은 `server.py` 프로세스에서 일어난다.
`SERVER_URL`만 공개 MCP 서버 주소로 바꾸면, 같은 코드로 7장의 공개 MCP까지 확장된다.

> **모델 참고**: 이 교재는 6주차와 동일하게 `claude-sonnet-4-6`으로 통일했다. 이 모델은
> `temperature=0`을 그대로 받으므로 6주차 코드와 설정이 일치한다. 만약 최신 모델
> (`claude-opus-5`·`claude-sonnet-5`)로 바꾸면 `temperature` 인자를 빼야 한다(400 오류 방지).

## 6.3 [심화·선택] 원시 클라이언트로 프로토콜 확인 — `client.py`

이 절은 **건너뛰어도 6.2의 에이전트 실습에는 지장이 없다.** MCP가 실제로 HTTP/JSON-RPC로 어떻게
도는지 눈으로 확인하고 싶을 때만 본다. 서버가 잘 떠 있는지 **연결·목록·호출만 짧게** 확인한다.

```python
# client.py — (심화) 서버 연결과 Tool을 원시 프로토콜로 확인만 한다
import asyncio
...
async def main() -> None:
    # URL 문자열을 넘기면 Streamable HTTP transport로 접속한다. server.py가 먼저 떠 있어야 한다.
    async with Client(SERVER_URL) as client:
        # 1) Tool 목록 확인
        tools = await client.list_tools()
        print("Tool 목록:", [t.name for t in tools])

        # 2) Tool 직접 호출 (사람이 인자를 지정)
        result = await client.call_tool("add", {"a": 7, "b": 8})
        print("add(7, 8) =", result.data)
...
```text
Tool 목록: ['add', 'calculate_score']
add(7, 8) = 15
```

핵심은 `client.py`가 `add()` 함수를 직접 import하지 않는다는 것이다. `call_tool("add", ...)` 요청이
HTTP로 서버에 가고, 실제 실행은 `server.py`에서 일어난다. 개념적 흐름은 다음과 같다.

```text
client.py:  await client.call_tool("add", {"a": 7, "b": 8})
                │  HTTP POST /mcp · JSON-RPC: tools/call
                ▼
server.py:  입력 검증 → add(7, 8) 실행 → 결과 반환
                │  HTTP 응답
                ▼
client.py:  result.data == 15
```

`localhost` URL을 실제 서버 URL로 바꾸면 같은 구조를 원격 MCP 서비스에도 적용할 수 있다. 실제
배포에서는 인증·TLS·접근 제어·Origin/Host 검증을 추가한다.

> **참고**: `list_resource_templates()`·`read_resource("course://AI-MASTER")`·`get_prompt(...)`로
> Resource·Prompt도 같은 방식으로 확인할 수 있다. 반환 객체의 형태(`result.data`, `result.content` 등)는
> SDK 버전에 따라 조금씩 다를 수 있으니, 값이 예상과 다르면 `result`를 그대로 출력해 구조를 확인한다.

## 6.4 실습 C — 6주차 도구(검색·RAG·메모리)를 MCP로 감싸기

5장의 `add`·`calculate_score`는 개념을 보여 주기 위한 장난감 Tool이었다. 이 절에서는 **6주차에
직접 만든 진짜 도구** — Tavily `web_search`, Chroma 기반 `document_search`(RAG), mem0 장기 기억 —
을 MCP 서버로 올린다. 그러면 6.2의 `agent_client.py`가 **코드를 거의 바꾸지 않고** 이 도구들을
그대로 소비한다. 이것이 "메모리 + RAG + 검색을 갖춘 실제 에이전트 앱"의 뼈대다.

### 핵심 원리 — `@tool`을 `@mcp.tool`로 바꾼다

6주차의 함수는 대부분 이미 `@tool`이 붙은 순수 파이썬 함수였다. **함수 본문은 그대로 두고
데코레이터만 `@mcp.tool()`로 바꾸면** MCP 서버가 된다. 아래 표가 전환의 전부다.

| 6주차 (앱 내부 Tool) | 7주차 (MCP Tool) |
|---|---|
| `@tool`<br/>`def web_search(...)` | `@mcp.tool()`<br/>`def web_search(...)` (본문 동일) |
| 앱과 같은 프로세스에서 실행 | 별도 서버 프로세스에서 실행 |
| 이 앱에서만 재사용 | 어떤 MCP Host·에이전트에서도 재사용 |

### `tools_server.py`

`mcp-demo`에 6주차 도구용 패키지를 추가한다. (6주차에서 이미 쓰던 것과 같다.)

```bash
uv add fastmcp tavily-python chromadb mem0ai python-dotenv
```

```dotenv
# .env  (커밋 금지 — .gitignore에 .env 추가)
ANTHROPIC_API_KEY=sk-ant-...
TAVILY_API_KEY=tvly-...
```

```python
# tools_server.py — 6주차의 검색·RAG·메모리 도구를 MCP Tool로 공개한다
import os
...
mcp = FastMCP(
    "AI MASTER Tools",
    instructions="6주차의 web_search·document_search·memory 도구를 MCP로 제공한다.",
)

# 6주차 Day2에서 쓰던 클라이언트/스토어를 그대로 준비한다.
...
# mem0 설정은 6주차 Day1에서 만든 것을 그대로 사용한다(모델은 claude-sonnet-4-6).
...
def document_search(query: str, top_k: int = 3) -> str:
    ...
    return "\n".join(f"- {d}" for d in docs) if docs else "관련 문서를 찾지 못했습니다."


@mcp.tool()
def remember(text: str, user_id: str = "student") -> str:
    """사용자가 명시적으로 기억해 달라고 한 사실을 장기 기억에 저장한다."""
    memory.add(text, user_id=user_id)
    return "기억에 저장했습니다."


@mcp.tool()
def recall(query: str, user_id: str = "student") -> str:
    """장기 기억에서 질문과 관련된 사실을 검색한다."""
    hits = memory.search(query, user_id=user_id).get("results", [])
    return "\n".join(f"- {h['memory']}" for h in hits) if hits else "관련 기억이 없습니다."


if __name__ == "__main__":
    # 5장의 server.py가 8000번을 쓰므로 이 서버는 8001번을 쓴다.
    mcp.run(transport="http", host="127.0.0.1", port=8001)
```

> `web_search`·`document_search`의 함수 본문은 6주차 2일차 `agent_graph.py`·RAG 실습의 코드와
> **똑같다.** `@tool` → `@mcp.tool()` 한 줄만 바뀐 것을 눈으로 확인하라. mem0의 `Memory.from_config`
> 인자도 6주차 1일차에서 만든 설정을 그대로 붙여 넣으면 된다.

### `agent_client.py`가 이 서버까지 함께 소비하도록 연결 추가

6.2의 `agent_client.py`에서 **연결 딕셔너리에 서버 하나만 추가**하면 끝이다. 그래프·LLM·나머지
코드는 손대지 않는다.

```python
    client = MultiServerMCPClient(
        {
            "ai_master": {"url": "http://localhost:8000/mcp", "transport": "streamable_http"},
            "tools": {"url": "http://localhost:8001/mcp", "transport": "streamable_http"},
        }
    )
    tools = await client.get_tools()   # add·calculate_score + web_search·document_search·remember·recall
```

이제 에이전트는 계산은 `calculate_score`, 최신 정보는 `web_search`, 강의자료 질문은
`document_search`, "이거 기억해"는 `remember`를 **스스로 골라** 호출한다. 실행 순서는 다음과 같다.

```bash
# 터미널 1
uv run python server.py          # 8000: add·calculate_score
# 터미널 2
uv run python tools_server.py    # 8001: web_search·document_search·remember·recall
# 터미널 3
uv run python agent_client.py
```

> **벡터 스토어(RAG) 배포 주의**: `chromadb.PersistentClient(path="./chroma_db")`는 로컬 폴더에
> 인덱스를 저장한다. Hugging Face Spaces·Streamlit Cloud 같은 **무료 호스팅은 재시작 때 디스크가
> 초기화**되므로, 매번 임베딩을 다시 만들지 않으려면 ① 미리 만들어 둔 `chroma_db` 폴더를
> 저장소에 함께 커밋(읽기 전용 사용)하거나, ② 호스팅형 벡터 DB(예: Qdrant Cloud, Pinecone)를
> 쓴다. 검색·메모리만 쓰는 앱이라면 벡터 스토어가 없어도 되므로 `document_search`만 빼면 된다.

## 6.5 실습 D — Gradio로 최소 챗 UI 붙이기

터미널 출력 대신 **웹 채팅창**에서 에이전트와 대화한다. `gr.ChatInterface` 하나면 되고,
6.2에서 만든 그래프를 그대로 재사용한다. (Streamlit보다 챗봇 UI가 짧고, 배포도 Hugging Face
Spaces에 바로 올릴 수 있다.)
...
```python
# app.py — Gradio 최소 챗 UI. §6.2(또는 6.4)의 에이전트 그래프를 그대로 재사용한다.
import gradio as gr
from dotenv import load_dotenv
from langchain_anthropic import ChatAnthropic
from langchain_core.messages import HumanMessage, SystemMessage
...
load_dotenv()
SYSTEM_PROMPT = "너는 MCP Tool을 쓰는 조교다. 계산·검색·기억이 필요하면 반드시 Tool을 호출하라."


async def build_graph():  # §6.2와 동일한 그래프 (서버 두 개를 함께 연결)
    client = MultiServerMCPClient(
        {
            "ai_master": {"url": "http://localhost:8000/mcp", "transport": "streamable_http"},
            "tools": {"url": "http://localhost:8001/mcp", "transport": "streamable_http"},
...
브라우저에 `http://127.0.0.1:7860`이 열리고, 채팅창에 질문하면 에이전트가 MCP Tool을 선택해
답한다. 핵심은 **UI 코드가 에이전트 로직을 전혀 모른다**는 점이다. `chat()` 함수는 그래프에
메시지를 넣고 마지막 답변만 꺼낼 뿐이다.

> **배포**: 이 `app.py`를 Hugging Face Spaces(Gradio SDK)에 올리면 웹에서 접속 가능한 챗봇이 된다.
> 이때 `.env`는 커밋하지 말고 Spaces의 **Secrets**에 `ANTHROPIC_API_KEY`·`TAVILY_API_KEY`를
> 등록한다. 단, MCP 서버(`server.py`·`tools_server.py`)도 같은 공간에서 함께 실행되도록 구성해야
> 하며(예: `app.py` 시작 시 하위 프로세스로 기동), 벡터 스토어는 위 6.4의 배포 주의를 따른다.

# 제7장. 바로 가져다 쓰는 공개 MCP 서버 카탈로그

> **이 장은 (확장·선택)이다.** 오늘의 핵심(내 MCP 서버 + 에이전트)은 5·6장에서 이미 완성했다.
> 이 장은 "여러 MCP를 하나의 에이전트에 연결한다"는 확장을 **보여주기 위한 것**이며, 공개 서버는
> 버전·환경 이슈가 있을 수 있으니(§7.5) 시간이 남거나 관심 있는 학생만 진행한다.
...
> **최소 성공 경로 (Node.js 불필요)**: 5·6장의 `server.py`·`client.py`·`agent_client.py`는 모두
> 순수 Python이라 Node 없이 오늘의 핵심을 완주한다. 7장의 공개 서버 중에서도 Fetch·Time은 `uvx`로
> 실행되어 npx가 필요 없다(§7.7 [필수] 셀).

### npx가 부담되면 — npx 없이 하는 3가지 방법

npx는 설치보다 **설치 후 PATH·커널 문제**로 막히는 경우가 많다. npx를 피하려면 아래 순서로 택한다.

1. **(가장 안정) 내가 만든 FastMCP 서버를 쓴다.** 5장 `server.py`에 파이썬 함수를 `@mcp.tool`로 더
   붙이고, 6.2의 `agent_client.py`가 그 툴을 골라 쓰게 한다. **Node·npx·uvx 모두 불필요**하고 버전
...
(선택) 클라우드 대신 로컬 무료 실행을 원하면 Ollama를 설치해 `ChatOllama`로 바꾼다(§7.8 참고).
...
## 7.7 셀 4~10 — MCP 서버를 한 셀씩 추가 (모두 인증키 불필요)

> **[필수]** Fetch·Time은 `uvx`로 실행되어 **Node.js가 필요 없다.** 이 둘만으로도 실습을 완주할 수 있다.
> **[선택 · Node 필요]** 나머지는 `npx`로 실행되므로 Node.js 18+가 있어야 한다. 없으면 건너뛴다.
> **막히면**: 공개 서버가 `ImportError`(버전 취약성)로 실패해도, 6.2의 자작 FastMCP 서버로 대체하면
> 학습 목표(에이전트가 MCP 툴을 스스로 선택)는 동일하게 달성된다. (§7.5 "npx 없이 하는 3가지 방법")
...
아래부터는 **Node.js가 있어야 실행되는 선택 확장**이다. Node가 없으면 이 셀들은 건너뛰고 §7.8로 넘어간다.
...
클라우드 LLM(`ChatAnthropic`)에 Tool 전체를 바인딩한다. (`.env`의 `ANTHROPIC_API_KEY` 사용 — §7.5에서 설치 완료)
...
- [ ] `agent_client.py`: 내 MCP 서버에 접속해 에이전트가 Tool을 스스로 선택·실행
- [ ] `tools_server.py`: 6주차 도구(검색·RAG·메모리)를 `@mcp.tool`로 감싼 서버
- [ ] `app.py`: Gradio 챗 UI에서 에이전트와 대화한 실행 화면
- [ ] (심화·선택) `client.py`: 원시 프로토콜로 연결·목록·호출 확인
