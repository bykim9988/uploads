...
세션 4의 두 방향이 **하나의 질문에서 만나는** 마무리 예제입니다. 6주차 1일차 A-7에서 만든
`youtube_search`(yt-dlp 기반, API Key 불필요)를 `server.py`로 이사시키고(방향 ①), 공개 MCP의
`get_transcript`(방향 ②)와 연쇄시킵니다.
...
```python
# server.py — Tool 추가 (6주차 1일차 A-7의 함수 그대로, 데코레이터만 @mcp.tool()로 교체)
@mcp.tool()
def youtube_search(query: str, max_results: int = 3) -> str:
    """키워드로 유튜브 영상을 검색해 제목·URL 목록을 반환한다."""
    opts = {"quiet": True, "extract_flat": True, "skip_download": True}
    with YoutubeDL(opts) as ydl:
        info = ydl.extract_info(f"ytsearch{max_results}:{query}", download=False)
    return "\n".join(
        f"- {e.get('title')}: https://www.youtube.com/watch?v={e['id']}"
...
- **`youtube_search`·`get_transcript`가 네트워크 오류를 내요** → 학교·회사 네트워크가
  유튜브를 차단하면 실패합니다. 개인 네트워크에서 시도하고, yt-dlp는 최신으로 유지하세요
