...
> 6주차의 진짜 도구로 교체한 뒤(세션 4) → **Gradio 채팅 UI**로 감쌉니다(세션 5).
...
| 세션 4 (50분) | 장난감 Tool을 6주차의 진짜 도구로 교체 | `server.py` 확장 |
...
## 세션 4 — 실습 C: 6주차 도구 이사 — `web_search` 하나만

이번 세션의 한 가지: **`@tool`을 `@mcp.tool()`로 바꾸면 6주차 도구가 그대로 MCP Tool이 됩니다.**
