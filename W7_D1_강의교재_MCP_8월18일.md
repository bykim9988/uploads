...
`client.py`부터 `async`/`await`가 등장한다. 6주차의 동기 호출과 1:1로 대응되니 새 문법 3가지만
기억하면 된다.

| 6주차 (동기) | 7주차 (비동기) |
|---|---|
| `def main():` | `async def main():` |
| `result = model.invoke(...)` | `result = await client.list_tools()` |
| `main()` | `asyncio.run(main())` |
| (없음) | `async with Client(url) as client:` — 연결을 자동으로 열고 닫음 |

규칙은 셋뿐이다. **① `async def` 안에서만 `await`를 쓴다. ② 최상위 실행은 `asyncio.run(main())`.
③ 연결 수명은 `async with`가 관리한다.** MCP 서버 통신은 네트워크 I/O라 비동기로 처리한다.
...
## 6.8 실습 C — 내 MCP 서버 툴로 에이전트 만들기

지금까지 `client.py`는 사람이 직접 `call_tool`을 호출했다. 이 절은 강의 목표인
**"공개·자작 MCP 툴을 LLM 에이전트가 스스로 골라 쓰게 만들기"**를 5장에서 만든 `server.py`로
그대로 실습한다. 6.2절의 원시 `client.py`는 **프로토콜을 이해하기 위한 코드**로 남겨 두고, 여기서는
그 위에 **에이전트 계층**을 얹는다.
...
- 흐름이 7장(여러 공개 MCP 연결)과 동일하다: **MCP 서버 → get_tools → LLM 바인딩 → LangGraph 루프**.
  차이는 서버가 원격 공개 서버냐, 방금 내가 만든 `server.py`냐 뿐이다.
...
async def main() -> None:
    ...
    # 3) agent ↔ tools 루프를 가진 최소 그래프를 만든다. (7장과 동일한 패턴)
...
`client.py`(직접 호출)와 `agent_client.py`(LLM이 선택)의 차이가 핵심이다. 앞의 것은 프로토콜을
이해하기 위한 것이고, 뒤의 것은 **에이전트가 MCP 툴을 스스로 골라 쓰는** 실제 목표에 해당한다.
`SERVER_URL`만 공개 MCP 서버 주소로 바꾸면, 같은 코드로 7장의 공개 MCP까지 확장된다.

> **비용 참고**: `claude-opus-5`는 품질이 높지만 호출 비용이 크다. 수업에서 여러 번 반복
> 실행한다면 `ChatAnthropic(model="claude-sonnet-5", ...)`로 바꾸면 더 저렴하다. 로컬 LLM으로
> 완전 무료 실행하려면 7장의 `ChatOllama` 방식을 사용한다.
...
1. **(가장 안정) 내가 만든 FastMCP 서버를 쓴다.** 5장 `server.py`에 파이썬 함수를 `@mcp.tool`로 더
   붙이고, 6.8의 `agent_client.py`가 그 툴을 골라 쓰게 한다. **Node·npx·uvx 모두 불필요**하고 버전
...
> **막히면**: 공개 서버가 `ImportError`(버전 취약성)로 실패해도, 6.8의 자작 FastMCP 서버로 대체하면
