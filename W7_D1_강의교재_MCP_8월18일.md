...
### 대안 — 로컬 LLM 대신 클라우드 LLM(ChatAnthropic) 사용

로컬 `gpt-oss:20b`는 실행에 큰 메모리(대략 13~16GB 이상)가 필요하다. 사양이 부족하거나 설치가
번거로우면 Ollama 대신 클라우드 LLM으로 이 셀 하나만 교체하면 된다. **그래프·Tool·나머지 셀은
그대로 유지**된다.

먼저 패키지를 설치하고 API Key를 준비한다.

```bash
uv add langchain-anthropic python-dotenv
```
