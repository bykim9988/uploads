...
> **문서 기준 버전**  
> 본 교재는 MCP 공식 규격 `2026-07-28`과 MCP Python SDK v2를 기준으로 작성하였다.
> MCP는 빠르게 개정되는 표준이므로 실제 실습 전 `uv run mcp version`과 공식 문서를 확인한다.
...
Python SDK v2의 `Client`는 `async with` 블록이 연결 수명주기를 관리한다.
...
Python SDK v2와 `mcp` CLI가 설치되어야 한다.
...
if __name__ == "__main__":
    ...
    # 기본 MCP endpoint는 /mcp이므로 전체 URL은
    # http://localhost:8000/mcp 이다.
    mcp.run(
...
| `mcp.run(transport="streamable-http", ...)` | localhost의 8000번 포트에서 HTTP MCP 서버 실행 |
...
uv add langgraph langchain-mcp-adapters langchain-ollama ipykernel
...
SDK v1 코드의 `FastMCP`, `ClientSession`, 초기화 방식과 SDK v2의 `MCPServer`, `Client`
예제를 혼용하지 않는다.
...
4. [MCP Python SDK v2](https://py.sdk.modelcontextprotocol.io/)
