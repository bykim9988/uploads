...
> **최소 성공 경로 (Node.js 불필요)**: `uvx`는 `uv`에 포함되어 있으므로 **Fetch·Time MCP만으로
> 추가 설치 없이** 실습을 끝까지 완주할 수 있다(§7.7 [필수] 셀). Weather·Finance·Filesystem·Memory·
> Playwright는 **Node.js 18+(`npx`)가 있어야 하는 선택 확장**이다.

(선택) Node.js 확장을 할 경우 `node --version`으로 18 이상을 확인하고, Playwright까지 하려면
`npx playwright install chromium`으로 브라우저를 설치한다. (선택) 클라우드 대신 로컬 무료 실행을
원하면 Ollama를 설치해 `ChatOllama`로 바꾼다(§7.8 참고).
