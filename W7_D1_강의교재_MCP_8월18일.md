...
async def main() -> None:
    ...
    # 2) 클라우드 LLM에 Tool을 바인딩한다.
    #    temperature는 최신 Claude 모델에서 지원되지 않으므로 설정하지 않는다.
...
> **비용 참고**: `claude-opus-5`는 품질이 높지만 호출 비용이 크다. 수업에서 여러 번 반복 실행한다면
> `ChatAnthropic(model="claude-sonnet-5", ...)`로 바꾸면 더 저렴하다.
...
model = ChatAnthropic(
    model="claude-opus-5",   # 수업 비용을 낮추려면 claude-sonnet-5 로 교체
    max_tokens=1024,
    # temperature는 최신 Claude 모델에서 지원되지 않으므로 설정하지 않는다.
)
model_with_tools = model.bind_tools(tools)
...
> **주의**: 6주차의 `ChatAnthropic(model="claude-sonnet-4-6", temperature=0)`처럼 `temperature`를
> 넘기면 최신 모델(`claude-opus-5`, `claude-sonnet-5`)에서는 400 오류가 난다. 최신 모델은 샘플링을
> 내부에서 관리하므로 `temperature`를 지정하지 않는다.
