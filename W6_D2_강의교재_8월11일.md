...
## 5.4 LangGraph 대표 아키텍처 패턴

LangGraph로 만드는 에이전트는 결국 **상태(State)를 공유하며 노드와 엣지로 실행 흐름을
설계**하는 일이다. 실무에서 반복적으로 나타나는 대표 패턴은 다음 여섯 가지다. 오늘 만든
그래프가 어느 패턴인지, 내일 만들 그래프가 어느 패턴인지 짚으며 보면 좋다.

### 그림 5-2. 패턴 1 — 순차형 (Pipeline)

작업을 정해진 순서로 실행한다. 가장 단순한 형태로, 4.1의 LCEL 직선 파이프라인과 같은
모양이다.
...
### 그림 5-3. 패턴 2 — 라우터형 (Router)

조건에 따라 다음 노드를 선택한다. Conditional Edge가 핵심이며, 5.3의 B-4
`langgraph_router.py`가 바로 이 패턴이다.
...
### 그림 5-4. 패턴 3 — 병렬형 (Parallel)

여러 작업을 동시에 수행한 뒤 통합한다. 서로 의존하지 않는 조사·분석 작업을 병렬로
돌려 시간을 줄이고, 마지막 노드에서 State에 모인 결과를 합친다. 실습 B-1
`simple_graph.ipynb`의 셀 8·9·10이 이 패턴이다.
...
### 그림 5-5. 패턴 4 — 반복·자기수정형 (Loop)

평가 결과가 기준 미달이면 다시 개선한다. 3일차 실습 B의 Evaluator 재시도 루프
(`multi_agent_with_evaluator.py`)가 이 패턴이다.
...
### 그림 5-6. 패턴 5 — 감독자 멀티에이전트 (Supervisor)

Supervisor가 전문 에이전트를 조정한다. 각 에이전트의 결과는 Supervisor에게 돌아오고,
Supervisor가 다음에 누구를 부를지 결정한다. 3일차 실습 A의
`multi_agent_supervisor.py`가 이 패턴이다.
...
### 그림 5-7. 패턴 6 — Human-in-the-Loop

중요 단계에서 사람이 검토·승인한다. 자동화 흐름 중간에 사람의 판단을 끼워 넣는
패턴으로, LangGraph에서는 체크포인터(4.4)로 그래프를 멈췄다가 사람의 입력을 받아
이어서 실행하는 방식으로 구현한다.
...
여섯 패턴 모두 재료는 같다 — **State · Node · Edge · Conditional Edge ·
Checkpoint**(4.2 참조). 어떤 복잡한 에이전트도 이 다섯 구성요소의 조합이며, 실제
서비스는 보통 여러 패턴을 섞어 쓴다(예: Supervisor 안에서 각 에이전트가 순차형,
전체 흐름 끝에 Human-in-the-Loop).
